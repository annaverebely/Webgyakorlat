...
<div class="card shadow-sm mb-4">
    ...
    async function loadInventory() {
        try {
            ...
            if(items.length === 0) {
                tbody.innerHTML = '<tr><td colspan="4" class="text-center">Nincs még eszköz a leltárban.</td></tr>';
                return;
            }
...
document.getElementById('addItemForm').addEventListener('submit', async function(e) {
    ...
    if (result.success) {
        document.getElementById('addItemForm').reset(); // Mezők ürítése
        loadInventory(); // Táblázat frissítése azonnal!
    } else {
        alert("Hiba: " + result.message);
    }
});
    // Amint betölt az oldal, futtassa le a függvényt
